...
EVOAR_DIR="${EVOAR_DIR:-$PROJECT_ROOT/EvoAR-v1}"
...
if [ -n "${CONDA_ENV:-}" ]; then
  ...
  echo "[run_c2i_platform] conda activate: $CONDA_ENV -> PYTHON_BIN=$PYTHON_BIN"
fi
...
echo "[run_c2i_platform] using python: $PYTHON_BIN"
echo "[run_c2i_platform] project dir: $EVOAR_DIR"
echo "[run_c2i_platform] MACA_PATH=$MACA_PATH"
echo "[run_c2i_platform] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>}"
echo "[run_c2i_platform] MUSA_VISIBLE_DEVICES=${MUSA_VISIBLE_DEVICES:-<unset>}"
...
MASTER_PORT="${MASTER_PORT:-${MLP_WORKER_0_PORT:-29502}}"
...
echo "[run_c2i_platform] dist env:"
...
if [ "$NNODES" -gt 1 ]; then
  exec "${_DIST_LAUNCHER[@]}" \
    ...
    autoregressive/train/train_c2i_grouped.py \
    "$@"
fi
...
exec "$PYTHON_BIN" autoregressive/train/train_c2i_grouped.py "$@"
