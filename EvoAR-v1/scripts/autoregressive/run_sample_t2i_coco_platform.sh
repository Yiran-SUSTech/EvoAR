...
if [ -n "${CONDA_ENV:-}" ]; then
  ...
  echo "[run_sample_t2i_coco_platform] conda activate: $CONDA_ENV -> PYTHON_BIN=$PYTHON_BIN"
fi
...
echo "[run_sample_t2i_coco_platform] using python: $PYTHON_BIN"
echo "[run_sample_t2i_coco_platform] project dir: $LLAMAGEN_DIR"
echo "[run_sample_t2i_coco_platform] MACA_PATH=$MACA_PATH"
echo "[run_sample_t2i_coco_platform] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>}"
echo "[run_sample_t2i_coco_platform] MUSA_VISIBLE_DEVICES=${MUSA_VISIBLE_DEVICES:-<unset>}"
...
MASTER_PORT="${MASTER_PORT:-${MLP_WORKER_0_PORT:-29504}}"
...
echo "[run_sample_t2i_coco_platform] dist env:"
...
_PROMPT_CSV="${PROMPT_CSV:-$LLAMAGEN_DIR/evaluations/t2i/coco_captions.csv}"
_SAMPLE_DIR="${SAMPLE_DIR:-samples_coco}"
