...
if [ -n "${CONDA_ENV:-}" ]; then
  ...
  echo "[run_sample_c2i_platform] conda activate: $CONDA_ENV -> PYTHON_BIN=$PYTHON_BIN"
fi
...
echo "[run_sample_c2i_platform] dist env:"
...
_SAMPLE_SCRIPT="$LLAMAGEN_DIR/autoregressive/sample/sample_c2i_ddp.py"
_VQ_CKPT="${VQ_CKPT:-$LLAMAGEN_DIR/pretrained_models/vq_ds16_c2i.pt}"
...
exec "$PYTHON_BIN" "$_SAMPLE_SCRIPT" --vq-ckpt "$_VQ_CKPT" "$@"
